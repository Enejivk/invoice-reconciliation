...
function App() {
  ...
  const loadInvoices = async () => {
    ...
    try {
      const res = await axios.get(`${API_BASE}/tenants/${selectedTenant}/invoices`)
  ...
  const deleteInvoice = async (id) => {
    ...
    try {
      await axios.delete(`${API_BASE}/tenants/${selectedTenant}/invoices/${id}`)
  ...
  const runReconciliation = async () => {
    ...
    setLoading(true)
    try {
      const res = await axios.post(`${API_BASE}/tenants/${selectedTenant}/reconcile?min_score=50.0`)
  ...
  const loadMatches = async () => {
    if (!selectedTenant) return
    try {
      const res = await axios.post(`${API_BASE}/tenants/${selectedTenant}/reconcile?min_score=0`)
  ...
  const loadTransactions = async () => {
    if (!selectedTenant) return
    // Note: This endpoint doesn't exist in REST API, but we can get from matches
    // For demo, we'll show transactions from matches
    try {
      await loadMatches()
    } catch (error) {
      // Ignore
  ...
  const confirmMatch = async (matchId) => {
    ...
    try {
      await axios.post(`${API_BASE}/tenants/${selectedTenant}/matches/${matchId}/confirm`)
  ...
  return (
    <div className="container">
      ...
      <div className="card">
        ...
        {selectedTenant && (
          <p style={{ color: '#9ca3af' }}>
            Working with: <strong style={{ color: '#f9fafb' }}>{tenants.find(t => t.id === selectedTenant)?.name}</strong>
          </p>
        )}
      </div>
            ...
            <tbody>
              {invoices.map(inv => (
                <tr key={inv.id}>
                  ...
                  <td>${parseFloat(inv.amount).toFixed(2)} {inv.currency}</td>
                  <td>{inv.invoice_date ? new Date(inv.invoice_date).toLocaleDateString() : 'N/A'}</td>
                  <td>
                    <span className={`badge badge-${inv.status === 'matched' ? 'success' : inv.status === 'open' ? 'warning' : 'info'}`}>
                      {inv.status}
                    </span>
                  </td>
                  <td>
            ...
            <div className="grid">
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) => setTransactionForm({...transactionForm, external_id: e.target.value})}
              ...
              <div className="form-group">
                ...
                <input
                  type="datetime-local"
                  value={transactionForm.posted_at.replace('Z', '').slice(0, 16)}
                  onChange={(e) => setTransactionForm({...transactionForm, posted_at: e.target.value + ':00Z'})}
                  required
                />
              </div>
            </div>
      ...
      {/* Reconciliation Tab */}
      <div className={`tab-content ${activeTab === 'reconcile' ? 'active' : ''}`}>
        <div className="card">
          <h2>Run Reconciliation</h2>
          <p style={{ marginBottom: '15px', color: '#9ca3af' }}>
            This will automatically match invoices to bank transactions based on amount, date, and description similarity.
          </p>
          <button
        ...
        <div className="card">
          ...
          {matches.length === 0 ? (
            <p style={{ color: '#9ca3af', textAlign: 'center', padding: '20px' }}>
              No matches found. Run reconciliation first or create invoices and transactions.
            </p>
          ) : (
            <div className="grid">
              {matches.map(match => {
                ...
                return (
                  <div key={match.id} className={`match-card ${match.status === 'confirmed' ? 'confirmed' : ''}`}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                      <h3>Match #{match.id}</h3>
                      <span className={`score ${getScoreClass(parseFloat(match.score))}`}>
                        Score: {parseFloat(match.score).toFixed(1)}/100
                      </span>
                    </div>
                    <p><strong>Invoice:</strong> {invoice?.invoice_number || `#${match.invoice_id}`} - ${parseFloat(invoice?.amount || 0).toFixed(2)}</p>
                    <p><strong>Transaction:</strong> #{match.bank_transaction_id}</p>
                    ...
                    {match.status === 'proposed' && (
                      <div style={{ marginTop: '15px' }}>
                        ...
                        <button
                          ...
                          onClick={async () => {
                            const explanation = await getExplanation(match.invoice_id, match.bank_transaction_id)
                            alert(`Explanation: ${explanation.explanation}\n\nConfidence: ${explanation.confidence || 'N/A'}`)
                          }}
                        >
                          Get Explanation
                        </button>
